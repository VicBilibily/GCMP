...
import { Logger, ModelScopeApiClient, ApiKeyManager, ModelScopeHandler } from '../utils';
...
export class ModelScopeDynamicProvider {
    ...
    private genericProvider: GenericModelProvider | null = null;
    private modelScopeHandler: ModelScopeHandler;
    ...
    constructor() {
        // 创建 ModelScope Handler
        this.modelScopeHandler = new ModelScopeHandler(
    ...
    private async getEffectiveModels(): Promise<ModelConfig[]> {
        ...
        Logger.warn('无法获取 ModelScope 模型，使用默认模型列表');
        return await this.getDefaultModels();
    }    /**
     * 获取默认模型列表
     */
    private async getDefaultModels(): Promise<ModelConfig[]> {
        try {
            // 尝试从 package.json 获取默认模型
            const { extensions } = await import('vscode');
            const extension = extensions.getExtension('vicanent.gcmp');
            if (extension?.packageJSON?.configProvider?.modelscope?.models) {
                return extension.packageJSON.configProvider.modelscope.models;
            }
        } catch (error) {
            Logger.warn('无法从 package.json 获取 ModelScope 默认模型:', error);
        }
        ...
        return [
            {
                ...
                tooltip: 'ModelScope 通义千问3-Next-80B-A3B-Instruct - 80B参数的强大推理模型',
            ...
            },
            {
                ...
                tooltip: 'ModelScope 通义千问3-Coder-480B-A35B-Instruct - 专业代码生成模型',
                maxInputTokens: 256000,
                maxOutputTokens: 64000,
                version: 'Qwen/Qwen3-Coder-480B-A35B-Instruct',
                capabilities: {
                    toolCalling: true,
                    imageInput: false
                }
    ...
    private async createDynamicProviderConfig(): Promise<ProviderConfig> {
        ...
        return {
            displayName: ModelScopeDynamicProvider.DISPLAY_NAME,
            baseUrl: ModelScopeDynamicProvider.BASE_URL,
    ...
    async provideLanguageModelChatInformation(
        ...
        try {
            ...
            Logger.info(`ModelScope 获取到动态配置，模型数量: ${dynamicConfig.models.length}`);

            // 创建或更新内部的 GenericModelProvider
            if (!this.genericProvider) {
                this.genericProvider = new GenericModelProvider(
                    ModelScopeDynamicProvider.PROVIDER_KEY,
                    dynamicConfig
                );
            } else {
            ...
            const modelInfos = dynamicConfig.models.map(model => this.modelConfigToInfo(model));
    ...
    async provideLanguageModelChatResponse(
        ...
        try {
            ...
            // 直接使用 ModelScopeHandler 处理请求
            await this.modelScopeHandler.handleRequest(model, messages, options, progress, token);
    ...
    async provideTokenCount(
        ...
        // 如果有内部的 GenericModelProvider，使用其 token 计数功能
        if (this.genericProvider) {
