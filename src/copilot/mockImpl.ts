/*---------------------------------------------------------------------------------------------
 *  此文件包含为创建 NES 必要的参数而实现的类，但只是空实现，没有实际使用。
 ...
 *
 *  参考: getInlineCompletions.spec.ts 中的 TestAuthService 和 TestTelemetrySender
 *  参考: nesProvider.spec.ts 中的 TestCopilotTokenManager 和 TestTelemetrySender
 *--------------------------------------------------------------------------------------------*/

import type { AuthenticationGetSessionOptions, AuthenticationSession } from 'vscode';
import { IAuthenticationService, ITelemetrySender } from '@vscode/chat-lib';
