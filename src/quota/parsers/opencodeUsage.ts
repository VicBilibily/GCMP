...
export function buildOpenCodeUsageSummary(data: OpenCodeUsageData): string {
    ...
    return rolling && rolling.usedPercent > 0 && primaryCandidates.length > 0 ?
            `${primaryRemaining.toFixed(0)}% (${rolling.remainingPercent.toFixed(0)}%)`
        :   `${primaryRemaining.toFixed(0)}%`;
}

export function formatOpenCodeStatusBarText(icon: string, data: OpenCodeUsageData): string {
    return `${icon} ${buildOpenCodeUsageSummary(data)}`;
}
