...
/**
 * 扩展的 TokenRequestLog，提供向后兼容的便捷访问方法
 */
export interface ExtendedTokenRequestLog extends TokenRequestLog {
    /** 获取实际输入token数（解析 rawUsage） */
    getActualInput(): number;
    /** 获取缓存读取token数（解析 rawUsage） */
    getCacheReadTokens(): number;
    /** 获取输出token数（解析 rawUsage） */
    getOutputTokens(): number;
    /** 获取完整的解析结果 */
    getParsedTokens(): ParsedUsageTokens;
}
...
export class UsageParser {
    ...
    static extendLog(log: TokenRequestLog): ExtendedTokenRequestLog {
        ...
        return {
            ...log,
            getActualInput: () => parsed.actualInput,
            getCacheReadTokens: () => parsed.cacheReadTokens,
            getOutputTokens: () => parsed.outputTokens,
            getParsedTokens: () => parsed
        };
    }
