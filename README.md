...
### 3. 配置 VS Code 后台实用模型（可选）

VS Code 在后台使用轻量级模型执行标题生成、提交信息创建、搜索、意图检测等**实用任务**。默认使用 Copilot 内置模型会消耗月度额度（尤其是免费用户的有限配额）。将这些任务指向 GCMP 提供的模型，可节省 Copilot 额度给更重要的用途。
...
```json
{
    // 探索性子代理默认模型（Agent 模式中代码库探索与搜索时使用）
    "chat.exploreAgent.defaultModel": "MiniMax-M2.7-HighSpeed (TokenPlan) (gcmp.minimax)"
}
```

> 提示：在 `settings.json` 中编辑时，将光标置于值位置，使用 VS Code 智能提示从已注册的模型中选择即可。**若未配置**这些设置，VS Code 会使用 Copilot 内置模型执行实用任务，**这可能会消耗免费用户的 Copilot 月度额度**。推荐配置为 GCMP 模型，可避免实用任务占用 Copilot 配额。
...
- **按量计费(PayGo)**：**MiMo-V2.5-Pro**、**MiMo-V2.5**、**MiMo-V2-Flash**
...
- 代理优先级为：`model.proxy` → `gcmp.providerOverrides.<provider>.proxy` → `gcmp.proxy` → VS Code `http.proxy` → 环境变量（`HTTPS_PROXY` / `HTTP_PROXY`）→ **系统代理（自动检测）**。
...
GCMP 支持通过 `gcmp.providerOverrides` 配置项来覆盖提供商的默认设置，包括 `baseUrl`、`proxy`、`customHeader`、模型配置等。
...
> ⚠️ **功能冻结**：本插件的 Commit 生成提交消息功能**已冻结**，不再新增功能，保留当前实现供现有用户使用。可通过 `gcmp.commit.enabled` 设置开关控制。Copilot 已原生支持 Git 提交信息生成（在提交输入框右侧），迁移后 Copilot 使用 `chat.utilitySmallModel` 指定的模型，可按 [第 3 节](#3-配置-vs-code-后台实用模型可选) 配置 GCMP 模型。
...
鼠标悬停状态栏 Token 消耗图标，点击 tooltip 底部的「用 GitHub Gist 同步 API Key」快速进入，或通过 VS Code 命令面板执行 `GCMP: 用 GitHub 同步 API Key`。首次使用时会弹出 GitHub 认证授权 `gist` 作用域。授权完成后显示同步操作菜单（按组分隔）：

| 分组         | 操作                                                                              |
| ------------ | --------------------------------------------------------------------------------- |
| **同步操作** | **上传 API Key** — 加密上传至 GitHub Gist / **下载 API Key** — 从 Gist 恢复到本地 |
| **云端管理** | **管理云端 API Key** — 查看和删除 Gist 中的 API Key                               |
| **安全设置** | **设置/更改加密口令** / **清除加密口令** — 管理自定义加密口令                     |
