...
1. 在 VS Code 中执行命令 `GCMP: 用 GitHub 同步 API Key`
2. 首次使用时会弹出 GitHub 认证授权，授权 `gist` 作用域
3. 授权完成后显示同步操作菜单：
   - **上传 API Key** — 将本地已配置的 API Key 加密上传至 GitHub Gist
   - **下载 API Key** — 从 GitHub Gist 下载并恢复到本地
   - **管理云端 API Key** — 查看和删除已存储在 Gist 中的 API Key
   - **设置/更改加密口令** — 添加自定义口令增强加密保护
   - **清除加密口令** — 移除自定义口令（谨慎操作）

> 上传和下载时支持**按提供商选择**（默认全选），部分上传时不会覆盖远端未选中的密钥。
