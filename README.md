...
- **0.19.0 版本**：将引入官方配置体验及多ApiKey分组配置模式，提供更灵活的密钥管理和配置方式。
- **0.19.x 版本周期**：现有提供商配置模式将继续保持可用，确保平滑过渡。
    - `OpenAI / Anthropic Compatible` 在确认新模式兼容前，暂不进行调整，继续保留现有配置。
- **0.20.0 版本**：将移除当前的内置提供商的配置模式，全面切换至新的配置体系。
