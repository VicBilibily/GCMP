...
### 3. 配置 VS Code 后台实用模型与 GCMP 辅助模型（可选）
...
- **TTFT / TPOT 与输出速度**：请求记录中展示首 Token 延迟（TTFT）、输出耗时（TPOT）和平均输出速度，方便评估模型响应性能
...
- 支持选择内置提供商模型，也支持选择已配置图像输入能力的 Compatible Provider 模型。
