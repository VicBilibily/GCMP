...
智谱AI内联代码补全功能现已支持！使用 GLM-4.5-air 模型提供实时代码建议：

1. 在设置中启用：`"gcmp.inlineCompletion.enabled": true`
2. 配置智谱AI API密钥（使用命令：`GCMP: 设置 智谱AI API密钥`）
3. 在支持的编程语言文件中开始编码，系统会自动显示补全建议
4. 按 `Tab` 接受补全，按 `Esc` 拒绝
