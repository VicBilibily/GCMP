...
- **0.20.7：临时兼容项**：针对 VS Code 模型选择器显示模型不全的问题，新增 `gcmp.autoPrefixModelId` 配置项作为临时解决方案。
    - **问题原因**：VS Code 1.111.0 引入模型折叠模式后，当多个提供商存在相同模型 ID 时，选中模型的过滤逻辑异常（选中后模型移至顶部，下方列表过滤掉该 ID，导致同 ID 的其他提供商的模型无法显示）。
