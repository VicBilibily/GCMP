...
```json
{
    "gcmp.proxy": "http://127.0.0.1:7890", // 全局代理（可选）
...
- 代理优先级为：`model.proxy` → `gcmp.providerOverrides.<provider>.proxy` → `gcmp.proxy` → VS Code `http.proxy` → 环境变量（`HTTPS_PROXY` / `HTTP_PROXY`）。
