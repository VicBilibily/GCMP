...
- **摩尔线程**：移除摩尔线程 AI Coding Plan 提供商 (服务长期不可用)

### 调整

- **火山方舟**：Kimi-K2.5 新增多模态输入支持（`imageInput: true`）

## [0.18.29] - 2026-02-10

### 修复

- **Kimi状态栏**：适配官方用量查询接口调整
    - 接口响应格式变更：支持百分比模式（limit=100）和Token模式（limit≠100）
    - 状态栏显示调整：百分比模式显示百分比，Token模式显示格式化的Token数量（如 23.9M）
    - Tooltip表格调整：Token模式下显示更详细的信息（上限、已用、剩余、重置时间）

## [0.18.28] - 2026-02-10

### 修复

- **智谱AI状态栏**：适配官方用量查询接口调整
    - 接口响应格式变更：`TOKENS_LIMIT` 类型不再返回 `usage`、`currentValue`、`remaining` 字段，仅保留 `percentage` 和 `nextResetTime`
    - 状态栏显示调整：当接口不返回详细数值时，显示剩余百分比（余量）而非已使用百分比
    - Tooltip 表格调整：统一显示剩余百分比，与状态栏保持一致

## [0.18.27] - 2026-02-10

### 调整

- **Token 统计 - 请求记录**：调整首Token延迟列显示
    - 新增流式输出总耗时显示，格式为"延迟+耗时"（如：`245ms+1.5s`）
        - 延迟：从请求发送到首个Token返回的时间
        - 耗时：从首个Token到流结束的时间
...
- **CLI认证提供商**(实验性)：调整以使得配置流程初步兼容 VS Code 1.109.0

## [0.18.23] - 2026-02-05

### 新增

- **iFlow**：新增 **Kimi-K2.5** 系列模型支持
    - 目前未正式上线，存在较多问题，上下文暂时配置为 128K

### 完善

- **提供商配置**：统一优化各提供商的非静默模式配置向导触发机制以兼容 VS Code 1.109.0

### 依赖更新

- **VS Code API**：同步更新至 VS Code 1.109.0 API 定义

## [0.18.22] - 2026-02-04

### 新增

- **阿里云百炼**：新增 Coding Plan 套餐专属模型 **Qwen3-Max** (Thinking)
