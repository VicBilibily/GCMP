...
- **Codex App Server 传输（实验性）**：Codex 通道新增 `transport: "appServer"` 模式，启动本机 `codex app-server` 进程经 JSON-RPC 完成对话、模型发现与用量查询，扩展不再持有 OAuth 令牌（认证交给本机 codex CLI）；支持子配置 `codexBinary` / `idleShutdownMinutes` / `threadMode`（`ephemeral` 独立会话 / `persistent` 跨轮复用 thread 增量发送）；要求 codex CLI ≥ 0.153.4；沙盒固定 `read-only`、审批固定 `never`；VS Code 工具经 Dynamic Tools 桥接执行。子请求（标题/摘要等）自动降推理档位并关闭推理摘要；turn 失败按结构化 `codexErrorInfo` 接入统一重试分类（配额耗尽/超上下文不重试，限流/过载自动重试）；握手声明通知精简清单，并限制同一进程并发 turn 数；历史中的工具调用与结果以 `function_call` / `function_call_output` 注入 codex 上下文，`persistent` 模式工具集变化时自动新建会话。进程退出会结束在途 turn 并释放并发槽；配置切换会重建 app-server 进程。 [#404](https://github.com/VicBilibily/GCMP/issues/404)

---

### Added

- **Codex App Server transport (experimental)**: The Codex channel gains a `transport: "appServer"` mode that launches a local `codex app-server` process and handles chat, model discovery, and quota queries over JSON-RPC, so the extension no longer holds an OAuth token (authentication is delegated to the local codex CLI). Sub-options: `codexBinary` / `idleShutdownMinutes` / `threadMode` (`ephemeral` per-request sessions / `persistent` cross-turn thread reuse with incremental sends). Requires codex CLI ≥ 0.153.4. The sandbox is pinned to `read-only` and approvals to `never`; VS Code tools are bridged via Dynamic Tools. Sub-requests (titles, summaries, etc.) automatically lower reasoning effort and disable reasoning summaries; turn failures are mapped from structured `codexErrorInfo` into the unified retry classification (no retry on exhausted quota or context overflow, automatic retry on rate limits/overload); the handshake declares an opt-out notification list and caps concurrent turns per process; historical tool calls and results are injected into the codex context as `function_call` / `function_call_output` items, and `persistent` mode starts a fresh thread when the tool set changes across turns. Process exit finishes in-flight turns and frees concurrency slots; changing the transport config rebuilds the app-server process. [#404](https://github.com/VicBilibily/GCMP/issues/404)
