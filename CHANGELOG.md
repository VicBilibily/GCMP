...
- **模型编辑器**：新增从 API 获取模型列表功能
    - 在"请求模型ID"字段旁新增"获取模型"按钮，点击后自动从 BASE URL 获取可用模型列表
    - 支持多种 API 响应格式解析（OpenAI 标准格式、字符串数组、对象数组等）
...
- **智谱AI**：新增 `GLM-4.7-Flash` 系列模型，移除过时模型 `GLM-4.5-Flash`
    - 免费模型：`GLM-4.7-Flash`
    - 按量计费：`GLM-4.7-FlashX`
...
- **Commit 提交消息生成**：auto 模式会根据仓库最近提交历史自动推断提交消息使用的语言；历史语言不明确时回退为 `gcmp.commit.language`

## [0.17.2] - 2026-01-16

### 调整

- **Commit 提交消息生成**：`gcmp.commit.format` 新增 `auto` 并设为默认值
    - auto 会提供仓库级最近提交 subject（最近 50 条）作为上下文，让模型自动推断仓库提交风格（包含 emoji 前缀等）

## [0.17.1] - 2026-01-16

### 完善

- **Commit 提交消息生成**：完善文档与配置提示，提升使用引导体验
- **Compatible Provider**：补充 `gcmp.compatibleModels[*].sdkMode` 的实验性模式说明（OpenAI Responses / Gemini SSE）

## [0.17.0] - 2026-01-16

### 新增

- **Commit 消息生成**：新增 AI 驱动的提交消息生成功能（基于 VS Code Language Model API）
    - 新增命令：`生成提交消息`、`生成提交消息 - 暂存的更改`、`生成提交消息 - 未暂存的更改`
    - SCM 视图新增入口：在仓库标题栏按钮与更改(暂存)分组按钮中一键生成提交消息
    - 支持多仓库场景：优先根据 SCM 上下文推断仓库，无法推断时弹出选择
